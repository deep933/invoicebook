...
const tokenSchema = require('../models/schemas/token.schema')
...
const signInWithGoogle = async payload => {
  if (payload && payload.email) {
    const user = await userServices.getUserByEmail({ email: payload.email })
    if (user && user.authType === 'google') {
      await tokenServices.deleteAllUserTokens(user._id)
      return await tokenServices.generateAuthTokens(user)
    } else {
      if (user) {
